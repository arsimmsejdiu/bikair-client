...
import {stepManualLockWhiteStyles} from "@styles/TripStyles";

interface Props extends ViewProps {
    onStateChange: (state: TRIP_STEPS | null) => void
}

const ManualLockWhiteStep: React.FC<Props> = ({onStateChange}): React.ReactElement | null => {
    ...
    useEffect(() => {
        dispatch(onTripStepEvent(TRIP_STEPS.TRIP_STEP_MANUAL_LOCK_WHITE));
    }, []);

    return (
        <View style={{...stepManualLockWhiteStyles.container}}>
            <ScrollView>
                <GoBackCross onClick={handleBackAction}/>
                <View style={{marginTop: 30 + insets.top, marginBottom: 30 + insets.bottom}}>
                    <StatusBar backgroundColor={COLORS.white}/>
                    <Animated.Text style={[stepManualLockWhiteStyles.title, {
                        transform: [{translateX: viewAnimation}],
                        opacity: translationAnimation,
                    }]}>
                        {t("trip_process.manual_lock_white.title")}
                    </Animated.Text>

                    <Animated.Text style={[stepManualLockWhiteStyles.subTitle, {
                        transform: [{translateX: textAnimation}],
                        opacity: translationAnimation,
                    }]}>
                        {t("trip_process.manual_lock_white.disclaimerLock")}
                    </Animated.Text>

                    <TextButton
                        buttonContainerStyle={stepManualLockWhiteStyles.buttonContainerStyle}
                        label={t("trip_process.manual_lock_white.support") ?? "Contacter le Support"}
                        actionLabel={"MANUAL_LOCK_WHITE_SUPPORT"}
...
export const StepManualLockWhite = memo(ManualLockWhiteStep);
